...
namespace payzen_backend.Controllers.SystemData
{
    ...
    public class CountriesController : ControllerBase
    {
        ...
        public async Task<ActionResult<CountryReadDto>> Create([FromBody] CountryCreateDto dto)
        {
            ...
            // Vérifier que le code pays n'existe pas déjà
            if (await _db.Countries.AnyAsync(c => c.CountryCode == dto.CountryCode && c.DeletedAt == null))
                return Conflict(new { Message = "Un pays avec ce code existe déjà" });
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            if (hasCompanies)
                return BadRequest(new { Message = "Impossible de supprimer ce pays car il est utilisé par des sociétés" });
