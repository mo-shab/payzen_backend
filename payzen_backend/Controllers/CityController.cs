...
namespace payzen_backend.Controllers
{
    ...
    public class CityController : ControllerBase
    {
        ...
        public async Task<ActionResult<CityReadDto>> Create([FromBody] CityCreateDto dto)
        {
            ...
            // Vérifier qu'une ville avec ce nom n'existe pas déjà dans ce pays
            if (await _db.Cities.AnyAsync(c => c.CountryId == dto.CountryId && c.CityName == dto.CityName && c.DeletedAt == null))
                return Conflict(new { Message = "Une ville avec ce nom existe déjà dans ce pays" });
        ...
        public async Task<IActionResult> Delete(int id)
        {
            ...
            // Vérifier si la ville est utilisée par des sociétés
            var hasCompanies = await _db.Companies.AnyAsync(c => c.CityId == id && c.DeletedAt == null);
            if (hasCompanies)
                return BadRequest(new { Message = "Impossible de supprimer cette ville car elle est utilisée par des sociétés" });
